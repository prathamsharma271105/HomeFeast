...
export const AuthModal = () => {
  ...
  const handleFastDemoLogin = async (demoRole) => {
    ...
    try {
      setLoading(true);
      const res = await api.login(email, 'password123', demoRole);
      const userObj = res.user || res.data;
      if (res.success && userObj) {
        loginUser(userObj);
        addToast(`Signed in as Demo ${label}! 🚀`, 'info');
      } else {
        addToast(res.message || 'Demo login failed.', 'error');
      }
    } catch (err) {
      addToast('Network error during demo login.', 'error');
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    ...
    >
      ...
      >
        {/* Modal Top Banner */}
        <div style={{ background: 'linear-gradient(135deg, #E8590C 0%, #FA8C16 100%)', padding: '24px 24px 18px 24px', color: '#FFFFFF', position: 'relative' }}>
          <button
            onClick={() => setIsAuthModalOpen(false)}
            style={{
              position: 'absolute',
          ...
          <div style={{ display: 'flex', background: 'rgba(0, 0, 0, 0.15)', borderRadius: '12px', padding: '4px', marginTop: '16px' }}>
            <button
              ...
              style={{
                ...
                color: activeTab === 'login' ? '#E8590C' : '#FFFFFF',
                fontWeight: 700,
                fontSize: '14px',
                cursor: 'pointer',
            ...
            <button
              ...
              style={{
                ...
                color: activeTab === 'register' ? '#E8590C' : '#FFFFFF',
                fontWeight: 700,
                fontSize: '14px',
                cursor: 'pointer',
                transition: 'all 0.2s ease'
        ...
        {/* Instant 1-Click Demo Accounts Switcher */}
        <div style={{ background: '#FFF9F2', padding: '14px 20px', borderBottom: '1px solid #EAE3D9' }}>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '8px' }}>
            <span style={{ fontSize: '11px', fontWeight: 800, color: '#78716C', textTransform: 'uppercase', letterSpacing: '0.08em', display: 'flex', alignItems: 'center', gap: '4px' }}>
              <Sparkles size={13} color="#E8590C" /> Fast 1-Click Demo Accounts:
            </span>
            <span style={{ fontSize: '11px', color: '#A8A29E' }}>password123</span>
          </div>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(105px, 1fr))', gap: '6px' }}>
            <button
              ...
              disabled={loading}
              data-testid="demo-login-customer"
              style={{
                padding: '8px 6px',
                borderRadius: '10px',
            ...
            >
              <User size={13} color="#E8590C" />
              <span>Customer</span>
            </button>
            <button
              type="button"
              onClick={() => handleFastDemoLogin('PROVIDER')}
              disabled={loading}
              data-testid="demo-login-provider"
              style={{
                padding: '8px 6px',
                borderRadius: '10px',
                border: '1.5px solid #EAE3D9',
            ...
            >
              <ChefHat size={13} color="#2B8A3E" />
              <span>Home Cook</span>
            </button>
            <button
              type="button"
              onClick={() => handleFastDemoLogin('RIDER')}
              disabled={loading}
              data-testid="demo-login-rider"
              style={{
                padding: '8px 6px',
                borderRadius: '10px',
                border: '1.5px solid #EAE3D9',
