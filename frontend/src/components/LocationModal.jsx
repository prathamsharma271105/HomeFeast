      ...
      >
                ...
                <div
                  ...
                  style={{
                    background: isSelected
                      ...
                      : '#FFFFFF',
                    border: `1.5px solid ${isSelected ? '#E8590C' : '#EAE3D9'}`,
                    borderRadius: '14px',
                    padding: '12px',
                    cursor: 'pointer',
                    color: isSelected ? '#FFFFFF' : '#1C1917',
                    boxShadow: isSelected
                      ? '0 6px 20px -3px rgba(232, 89, 12, 0.35)'
        ...
        <div style={{ padding: '20px 24px', flexGrow: 1, overflowY: 'auto', maxHeight: '380px' }}>
          ...
          <div style={{ fontSize: '12px', fontWeight: 700, color: '#44403C', marginBottom: '10px', display: 'flex', justifyContent: 'space-between' }}>
            ...
            <span style={{ color: '#E8590C' }}>{filteredLocalities.length} Areas</span>
          </div>

          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(180px, 1fr))', gap: '8px', marginBottom: '20px' }}>
            {filteredLocalities.map((loc) => {
              const isCurrent = selectedCity === currentCityObj.id && selectedLocality.toLowerCase().includes(loc.toLowerCase().split('(')[0].trim());
              return (
                <div
                  key={loc}
                  ...
                  style={{
                    padding: '10px 12px',
                    borderRadius: '10px',
                    border: `1px solid ${isCurrent ? '#E8590C' : '#EAE3D9'}`,
                    background: isCurrent ? '#FFF4E6' : '#FFFFFF',
                    color: isCurrent ? '#E8590C' : '#1C1917',
                  ...
                  onMouseEnter={(e) => {
                    if (!isCurrent) {
                      ...
                      e.currentTarget.style.background = '#FFF9F2';
                    }
                  }}
                  onMouseLeave={(e) => {
                    if (!isCurrent) {
                ...
                >
                  ...
                  {isCurrent ? <Check size={14} color="#E8590C" /> : <MapPin size={12} color="#A8A29E" />}
                </div>
              );
            })}
          </div>
