...
export const ReviewModal = () => {
  ...
  return (
    ...
    >
      ...
      >
        {/* Modal Header */}
        <div style={{ background: 'linear-gradient(135deg, #E8590C 0%, #F59E0B 100%)', padding: '24px', color: '#FFFFFF', position: 'relative' }}>
          <button
            onClick={() => setIsReviewModalOpen(false)}
            style={{
              position: 'absolute',
        ...
        <form onSubmit={handleSubmit} style={{ padding: '24px', display: 'flex', flexDirection: 'column', gap: '20px' }}>
            ...
            <div style={{ display: 'flex', justifyContent: 'center', gap: '8px' }}>
              {[1, 2, 3, 4, 5].map(star => (
                ...
                >
                  <Star
                    size={36}
                    color={(hoverRating || rating) >= star ? '#F59E0B' : '#EAE3D9'}
                    fill={(hoverRating || rating) >= star ? '#F59E0B' : 'none'}
                  />
                </button>
              ))}
            </div>
            <div style={{ fontSize: '14px', fontWeight: 700, color: '#E8590C', marginTop: '8px' }}>
              {rating === 5 && '🌟 Exceptional! Just like mom’s cooking'}
              {rating === 4 && '👍 Delicious & Wholesome'}
          ...
          <div>
            <label style={{ display: 'block', fontSize: '13px', fontWeight: 700, color: '#1C1917', marginBottom: '6px' }}>
              Your Review & Compliments <span style={{ color: '#E8590C' }}>*</span>
            </label>
            <textarea
              rows={4}
              value={comment}
