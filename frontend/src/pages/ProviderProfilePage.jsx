...
export const ProviderProfilePage = ({ providerId, onBack, onOpenThaliBuilder }) => {
  ...
  return (
    <div style={{ maxWidth: '1240px', margin: '0 auto', padding: '24px 16px 80px 16px' }}>
        ...
        <div style={{ position: 'relative', height: '240px', background: '#1C1917' }}>
          ...
          <div style={{ position: 'absolute', bottom: '20px', left: '24px', right: '24px', color: '#FFFFFF' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '4px' }}>
              <span style={{ background: '#E8590C', color: '#FFFFFF', padding: '2px 10px', borderRadius: '12px', fontSize: '11px', fontWeight: 800, textTransform: 'uppercase' }}>
        ...
        >
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
            ...
            <div>
              ...
              <div style={{ fontWeight: 800, color: '#E8590C' }}>₹{provider.startingPrice} / meal</div>
            </div>
          </div>
        </div>
      </div>
          ...
          return (
            <button
              ...
              style={{
                ...
                border: 'none',
                background: 'none',
                borderBottom: isActive ? '3px solid #E8590C' : '3px solid transparent',
                color: isActive ? '#E8590C' : '#57534E',
                fontWeight: 800,
                fontSize: '14px',
                cursor: 'pointer',
                marginBottom: '-2px',
            ...
            >
              <Icon size={18} color={isActive ? '#E8590C' : '#78716C'} />
              <span>{tab.label}</span>
            </button>
          );
        })}
            ...
            {categories.map(cat => (
              <button
                ...
                style={{
                  padding: '8px 16px',
                  borderRadius: '9999px',
                  border: selectedCategory === cat ? '1.5px solid #E8590C' : '1.5px solid #EAE3D9',
                  background: selectedCategory === cat ? '#FFF4E6' : '#FFFFFF',
                  color: selectedCategory === cat ? '#E8590C' : '#57534E',
                  fontWeight: 700,
                  fontSize: '13px',
                  cursor: 'pointer',
                ...
                >
                  ...
                  <div style={{ padding: '16px', display: 'flex', flexDirection: 'column', flexGrow: 1 }}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline', marginBottom: '6px' }}>
                      ...
                      <div style={{ fontSize: '18px', fontWeight: 900, color: '#E8590C' }}>
                        ₹{dish.price}
                      </div>
                    </div>
                    ...
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', borderTop: '1px solid #FAF8F5', paddingTop: '12px' }}>
                      ...
                      {qtyInCart > 0 ? (
                        <div style={{ display: 'flex', alignItems: 'center', gap: '8px', background: '#FFF4E6', borderRadius: '10px', padding: '4px 8px' }}>
                          <button
                            onClick={() => addItem(dish, -1, provider)}
                            style={{ width: '26px', height: '26px', borderRadius: '6px', border: 'none', background: '#E8590C', color: '#FFF', fontWeight: 800, cursor: 'pointer' }}
                          >
                            -
                          </button>
                          <span style={{ fontWeight: 800, fontSize: '14px', color: '#E8590C', minWidth: '16px', textAlign: 'center' }}>
                            {qtyInCart}
                          </span>
                          <button
                            onClick={() => addItem(dish, 1, provider)}
      ...
      {activeTab === 'plans' && (
        <div>
          <div style={{ marginBottom: '20px', background: '#FFF9F2', padding: '16px 20px', borderRadius: '16px', border: '1px solid #FDE68A' }}>
              ...
              return (
                <div
                  ...
                  style={{
                    background: '#FFFFFF',
                    borderRadius: '24px',
                    border: plan.planType === 'MONTHLY' ? '2px solid #E8590C' : '1px solid #EAE3D9',
                    padding: '24px',
                    display: 'flex',
                    flexDirection: 'column',
                ...
                >
                  ...
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '8px' }}>
                    <div>
                      <span style={{ fontSize: '11px', fontWeight: 800, color: '#E8590C', textTransform: 'uppercase', letterSpacing: '0.1em' }}>
                        {plan.planType} PASS
                      </span>
          ...
          >
            <div style={{ textAlign: 'center', borderRight: '1px solid #EAE3D9', paddingRight: '20px' }}>
              ...
              <div style={{ display: 'flex', justifyContent: 'center', gap: '4px', margin: '8px 0' }}>
                {[1, 2, 3, 4, 5].map(s => (
                  <Star key={s} size={20} color="#F59E0B" fill={s <= Math.round(provider.rating) ? '#F59E0B' : 'none'} />
                ))}
              </div>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
            {reviews.map(rev => (
              ...
              >
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '8px' }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
                    <img
                      ...
                      style={{ width: '40px', height: '40px', borderRadius: '50%', background: '#FFF4E6' }}
                    />
                    <div>
                  ...
                  <div style={{ display: 'flex', gap: '2px' }}>
                    {[1, 2, 3, 4, 5].map(s => (
                      <Star key={s} size={15} color="#F59E0B" fill={s <= rev.rating ? '#F59E0B' : 'none'} />
                    ))}
                  </div>
                </div>
                ...
                {rev.favoriteDish && (
                  <div style={{ fontSize: '12px', color: '#E8590C', fontWeight: 700 }}>
